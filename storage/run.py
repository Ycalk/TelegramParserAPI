...
def start_worker(worker_id: int, workers_count: int, functions, queue_name):
    ...
    logging_config['loggers']['storage'] = {'level': log_level, 'handlers': ['arq.standard']}
...
def main(max_workers: int):
    with ProcessPoolExecutor(max_workers=max_workers + 1) as executor:
        for i in range(max_workers):
            executor.submit(start_worker, i + 1, max_workers, FUNCTIONS, os.getenv('STORAGE_QUEUE_NAME', 'storage'))
