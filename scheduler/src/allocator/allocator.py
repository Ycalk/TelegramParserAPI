...
class Allocator:
    def __init__(self, slots_count: int, allocation_interval_minutes: int, channels: list[int] = []) -> None:
    ...
    def __create_slots(self) -> list[TimeSlot]:
        return [TimeSlot(datetime.now() + timedelta(minutes=self.allocation_interval_minutes * i)) for i in range(self.slots_count)]