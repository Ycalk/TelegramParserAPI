...
class CustomClient:
    def __init__(self, client: Client, redis_config: RedisConfig) -> None:
        self._redis = Redis(host=redis_config.host, port=redis_config.port, db=redis_config.db)
        self._client = client
    ...
    async def _get_client_from_tdata(self) -> TelegramClient:
        ...
        return await tdesk.ToTelethon(session=self._session, api=api, password=password, auto_reconnect=False) # type: ignore
    ...
    async def __aenter__(self) -> TelegramClient:
        ...
        if session_str:
            ...
            try:
                ...
                await Client.filter(id=self._client.id).update(users_count=F('users_count') + 1)
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb) -> None:
        if self._session:
            await self._redis.set(str(self._client.id), self._session.save().encode('utf-8'))
        if self._t_client:
            await Client.filter(id=self._client.id).update(users_count=F('users_count') - 1)