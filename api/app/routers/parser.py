...
async def add_client(tdata: UploadFile = File(...), api_key_verified: None = Depends(verify_api_key)):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail={"error": "SessionPasswordNeeded", "message": str(e)})
...
async def get_channel_info(request: GetChannelInfoRequest, api_key_verified: None = Depends(verify_api_key)):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail={"error": "InvalidChannelLink", "message": str(e)})
    except FloodWait as e:
        raise HTTPException(status_code=400, detail={"error": "FloodWait", "message": str(e)})
    except CannotGetChannelInfo as e:
        raise HTTPException(status_code=500, detail={"error": "CannotGetChannelInfo", "message": str(e)})
    except Exception as e:
